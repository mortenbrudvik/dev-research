...
|---|---|---|---|
| T1 | Ship an order: `POST /orders/{id}/ship`; `404` unknown, `409` if not pending, `200` with status; tests | slice-local, new use case | `Features/ShipOrder/**` + slice tests / `Application/Orders/Commands/ShipOrder/**` + `Api/Endpoints/OrdersEndpoints.cs` + integration tests |
| T2 | Shipped orders can no longer be cancelled: `409` with a clear detail; tests | slice-local change | `Features/CancelOrder/**`, possibly `Domain/CancellationPolicy.cs` + tests / `Application/Orders/Commands/CancelOrder/**`, possibly `Domain/Policies/**` + tests |
| T3 | List a customer's orders: `GET /customers/{customerId}/orders`, newest first; tests | slice-local query | `Features/ListOrdersByCustomer/**` + tests / `Application/Orders/Queries/ListOrdersByCustomer/**` + `Api/Endpoints/**` + tests |
| T4 | Record an audit entry (who, what, when, order id) for every command; `GET /orders/{id}/audit` lists them; tests | cross-cutting | `Platform/**`, `Domain/**`, every command slice, new `Features/GetOrderAudit/**` + tests / `Domain/**`, `Application/Common/**`, every command, `Infrastructure/**`, `Api/Endpoints/**` + tests |
| T5 | Add optional free-text `Notes` to an order, settable on create and returned on get; migration; tests | persistence | `Domain/Order.cs`, `Platform/Persistence/**` (migration), `Features/CreateOrder/**`, `Features/GetOrder/**` + tests / `Domain/**`, `Infrastructure/Persistence/**`, the two handlers, `Api/Endpoints/**` + tests |
