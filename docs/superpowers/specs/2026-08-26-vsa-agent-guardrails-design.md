...
| T2 | Shipped orders can no longer be cancelled: `409` with a clear detail; tests | slice-local change | `Features/CancelOrder/**`, possibly `Domain/CancellationPolicy.cs` + tests / `Application/Orders/Commands/CancelOrder/**`, `Api/Endpoints/OrdersEndpoints.cs`, possibly `Domain/Policies/CancellationPolicy.cs` + tests |
...
| T4 | Record an audit entry (who, what, when, order id) for every command; `GET /orders/{id}/audit` lists them; tests | cross-cutting | `Domain/**`, `Platform/**`, `Features/**`, `Program.cs`, `Orders.Api.csproj` + tests / `Domain/**`, `Application/**`, `Infrastructure/**`, `Api/Common/**`, `Api/Endpoints/**`, `Api/Program.cs` + tests — both include the composition root and the HTTP-concerns folder, both include the composition root and the API project file a package reference would touch, both exclude settings and launch profiles |
...
Every task's scope additionally includes the copy's `CLAUDE.md`: recording a new convention in the rules file is legitimate work in either architecture, and leaving it out charged an arm for a documentation note rather than for sprawl (observed in the T4 pilot, 2026-08-31).
