...
**Status:** current as of August 2026; section 10 updated with the results of the minimal-pair experiment run on 2026-08-31. The anchor talk was published on 2026-08-24; every statistic and tool version below is dated and sourced in [References](#11-references). Quotations from the talk are taken from its captions and may differ from the spoken wording by a word or two.
...
**The prototype, and what it found.** [`prototypes/ai-development/vsa-agent-guardrails/`](https://github.com/mortenbrudvik/dev-research/tree/main/prototypes/ai-development/vsa-agent-guardrails) is that experiment, built and run: a small .NET 10 Minimal API (four use cases, a shared domain model, SQLite through EF Core) laid out twice — once as vertical slices, once as Domain/Application/Infrastructure/Api — with behaviour tests identical file for file, and *both* copies carrying the mechanical half of this section: ArchUnitNET rules with negative controls, a rules file, a blocking Stop/PostToolUse hook, and a jscpd gate. Five tasks (three slice-local, one cross-cutting, one migration) went to Claude Code against each copy, three repetitions, one task per fresh copy, model pinned: 30 runs, $29.

All 30 finished green — build clean, no failing behaviour or architecture test, in both copies. Pooled over the three slice-local tasks the sliced copy read fewer distinct files (11 vs 12), ingested 5% fewer context tokens, re-read 24% less context per run, took 8% fewer turns and cost 19% less. The cross-cutting audit-trail task, which this guide predicted would erase the advantage, produced the widest margin instead: 22% lower cost, 18% fewer turns and 31% fewer files changed for the sliced copy. The migration task was a tie. Neither copy made a single edit outside the task's expected scope on four of the five tasks — with guardrails present in both, sprawl did not appear in either. Of 31 gate blocks across all runs, 28 were unintended build-stage failures on a half-written change — with warnings-as-errors set, a two-edit change trips the gate in either order — and the three that remain were one agent deliberately testing a rule it had just written. The numbers, the caveats that qualify them, and the raw per-run data are in [the experiment report](https://github.com/mortenbrudvik/dev-research/blob/main/prototypes/ai-development/vsa-agent-guardrails/experiment/REPORT.md).

Read it as one data point: a single codebase small enough that the layered copy's blast radius is still small, one model, three repetitions, and no significance testing. What it does show is that the effect runs in the direction the mechanisms predict, that it is modest rather than dramatic, and that the guardrails — not the folder layout — are what kept both copies inside their scope.

One event is worth reporting on its own, at n=1. The only architecture-rule block in all 30 runs was not a mistake being caught: in one layered run the agent, reading the task's "any operation added later" clause, added a rule of its own requiring every command handler to depend on the audit recorder, plus a companion test pinning that rule's patterns to the real handlers — then deleted the dependency to watch its new rule fail, and restored it. That is this section's loop with the agent on the other end of it, extending the guardrail rather than only obeying it. It happened once in thirty runs, unprompted and unrewarded: nothing in the harness scored durability, so the runs that answered the same clause with a comment came out identical.
